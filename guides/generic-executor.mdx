...
description: Attesso authorizes; CompanyX executes. A guide for any external API, queue, worker, or manual executor.
---

Attesso authorizes; CompanyX executes. This guide applies to any external API,
...
1. Create an immutable Mandate with a CompanyX subject reference, an external
...
9. Verify and retain the Attesso evidence bundle under CompanyX's own retention
   policy.
...
| Executor call | one CompanyX external reference/idempotency key |
...
An Authorization is a reservation, not proof that execution happened. CompanyX
...
Keep Attesso keys and executor credentials only on CompanyX's server. Do not send
...
cancel replay. Move to live only after billing is linked and the same CompanyX
...
payment credentials, or connect to CompanyX's payment account. Public support is
