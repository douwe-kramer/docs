...
1. Create an immutable Mandate with a integrator subject reference, an external
...
| Executor call | one the integrator external reference/idempotency key |
...
cancel replay. Move to live only after billing is linked and the same the integrator
